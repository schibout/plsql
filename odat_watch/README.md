...
| `plan_prod.py` / `ui_plan_prod.py` | **Plan de production** (Control-M › Plan de production) : la bible des chaînes `docs/Plan de Production.xlsx` (Lionel, juin 2025) chargée dans `pdp_chaines`, confrontée aux photos ODAT mois par mois de J-7 à J+16 (J = dernier jour ouvré du mois, jours ouvrés hors fériés, Control-M L/D en rappel) : grille chaînes × jours, chaînes nouvelles / jamais vues / plus vues, écarts de planification, planification observée à adopter, jalons de clôture, fiche chaîne (jobs, programmes Oracle, profils). |
