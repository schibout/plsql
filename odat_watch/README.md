...
| `controle_matin.py` | **Contrôle du matin** : portage `oracledb` des 17 contrôles de `ControleMatinGenerique/Controle_Quotidien_Complet.sql` (DSP, Notilus, factures Xerox/Tradeshift, factures AR reçues dans `DKA_IARPAFAC_INTERFACE` sur 24 h et rejetées par AutoInvoice, GL, traitements de la nuit, RB) sur une plage date+heure libre. Calcule les statuts OK/W et le statut global, historise la synthèse dans `controle_matin_histo`. Aussi en ligne de commande (`--rapport`). Le `.sql` reste la référence : toute évolution métier se fait d'abord là, puis se reporte ici. |
...
| `prelevements.py` / `ui_prelevements.py` | **Prélèvements** : onglet pont vers l'outil `../CTRL_QUASI_AUTOMATIQUE_DES_PRELEVEMENTS`, données et rapports dans `../ODAT/prelevements` (rapprochement Oracle ↔ EDF par clé IBAN créancier × échéance, `rapprochement_cle_metier.executer`) : date de référence et profondeur, tuiles (statut global, émis, en attente, anomalies, écarts à investiguer), justification des écarts, clés par statut, téléchargement CSV / classeur, contrôle des doublons d'émission, rapport HTML, mail `.eml`. Chaque lancement persiste les états EDF et les rejets lus (`pv_fichiers`, `pv_edf`, `pv_rejets`) et historise le rapprochement (`pv_histo`) : section « Trésorerie EDF » (chronologie, jours ouvrés sans état, rejets, mandats récidivistes, fichiers purgés du dossier). Config `[prelevements]`, `[mail]`. |
| `virements_import.py` | Import du dépôt `ODAT/virements/import_virement` : instances Talend, exports Quartz (sous-dossier `EDF/`) et rejets bancaires de virements (sous-dossier `REJET/`) déposés en vrac, datés par leur nom (préfixe `JJMMAAAA_` de l'Apps Script) ou leur contenu (noms DK_FIN01 / CSV Oracle, en-tête du classeur Quartz), rangés dans `JJMMAAAA/<uuid>`, `Liste des virements importés du jour<JJMMAAAA>.xls` et `REJETS/JJMMAAAA_*.xls` ; instances enregistrées dans `vir_imports`. L'onglet, le rapport HTML et le texte court reprennent les rejets du jour (`virements.lire_rejets`). |
...
| `plan_prod.py` / `ui_plan_prod.py` | **Plan de production** (Control-M › Plan de production) : la bible des chaînes `docs/Plan de Production.xlsx` (Lionel, juin 2025) chargée dans `pdp_chaines`, confrontée aux photos ODAT mois par mois de J-7 à J+16 (J = dernier jour ouvré du mois, jours ouvrés hors fériés, Control-M L/D en rappel) : grille chaînes × jours, chaînes nouvelles / jamais vues / plus vues, écarts de planification, planification observée à adopter, jalons de clôture, fiche chaîne (jobs, programmes Oracle, profils), bouton « Synchroniser avec ODAT ». Guide de lecture (couleurs, chiffres, synchro) : [`docs/PLAN_DE_PRODUCTION.md`](docs/PLAN_DE_PRODUCTION.md). |
| `ui_mode_emploi.py` | Sous-onglet Control-M › Mode d'emploi : légende des cellules dans les vraies couleurs de la grille, grille d'exemple commentée, écarts, origines, tuiles, planification, synchronisation, puis le guide complet `docs/PLAN_DE_PRODUCTION.md`. |
