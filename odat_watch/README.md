...
| `virements.py` / `ui_virements.py` | **Virements** : onglet pont vers l'outil `../controleVirement`, données dans `../ODAT/virements` (choix de la journée parmi les dossiers `*_cible`, lancement de `controle_virements.executer`, tuiles, doublons D1-D6, contrôles de forme, niveaux 0-3, synthèse). Config `[virements]`. |
