...
| `virements_import.py` | Import du dépôt `ODAT/virements/import_virement` : instances Talend et exports Quartz déposés en vrac, datés par leur contenu (noms DK_FIN01 / CSV Oracle, en-tête du classeur Quartz), rangés dans `JJMMAAAA/<uuid>` et enregistrés dans `vir_imports`. |
