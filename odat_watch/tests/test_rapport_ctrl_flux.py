...
def test_rapport_complet(tmp_path):
    ...
    h = rp.construire(e, lignes, cf.groupes_compenses(lignes), cf.rapprochements(con))
    assert "Ctrl Flux — export du 19/08/2026" in h
...
def test_bandeau_ko_si_ko(tmp_path):
    con, e, lignes = _contexte(tmp_path)
    lignes["statut"] = "OK"
    assert 'bandeau ok' in rp.construire(e, lignes, cf.groupes_compenses(lignes), cf.rapprochements(con))
    lignes.loc[lignes.index[0], "statut"] = "KO"
    assert 'bandeau ko' in rp.construire(e, lignes, cf.groupes_compenses(lignes), cf.rapprochements(con))
    con.close()


def test_ligne_orange_interface_en_attente(tmp_path):
    con, e, lignes = _contexte(tmp_path)
    assert "tr class='orange'" not in rp.construire(e, lignes, cf.groupes_compenses(lignes), cf.rapprochements(con))
    i = lignes.index[0]
...
def test_ligne_gdr(tmp_path):
    con, e, lignes = _contexte(tmp_path)
    lignes["gdr"] = ""
    assert "tr class='gdr'" not in rp.construire(e, lignes, cf.groupes_compenses(lignes), cf.rapprochements(con))
...
def test_ecrire(tmp_path):
    con, e, lignes = _contexte(tmp_path)
    p = rp.ecrire(e, lignes, cf.groupes_compenses(lignes), cf.rapprochements(con), tmp_path)
    assert p.name.startswith("Ctrl_Flux_20260819_") and p.read_text(encoding="utf-8").startswith("<!DOCTYPE html>")
    con.close()
