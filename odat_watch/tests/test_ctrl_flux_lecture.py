"""Lecture des exports Ctrl Flux (règles de Verifier_Factures.ps1) sur les fichiers réels du dépôt."""
...
def _lignes_donnees(chemin: Path) -> int:
    ...
    texte = octets.decode(cf.detecter_encodage(octets))
...
def test_regles_unitaires():
    assert cf.normaliser("App Amont Nb piéce ") == "app amont nb piece"
    assert cf.montant("1 234,56") == (1234.56, False)
    assert cf.montant("1 234,56") == (1234.56, False)
    assert cf.montant("-") == (0.0, False) and cf.montant("") == (0.0, False)
    assert cf.montant("abc") == (0.0, True)
    assert cf.montant("nan") == (0.0, True)
    assert cf.type_flux("HEF01_SRC_FACTURESCLIENTS_070826-011057_ST_HEF01_6392_001") == "CLIENTS"
    assert cf.type_flux("CEL01_SRC_FACTURESFOURNISSEURS_070826") == "FOURNISSEURS"
    assert cf.type_flux("CDPG_XXX") == "GL" and cf.type_flux("truc") == "AUTRE" and cf.type_flux("") == "AUTRE"
    assert cf.fichier_base("A_B_ST_HEF01_6392_001") == "A_B" and cf.fichier_base("A_B") == "A_B"
    assert cf.date_export_du_nom("ExportCSV-19-08-2026_16h00.csv") == date(2026, 8, 19)
    assert cf.date_export_du_nom("autre.csv") is None
    assert cf.date_export_du_nom("ExportCSV-32-13-2026.csv") is None


def test_empreinte_stable_entre_deux_exports():
    a = cf.lire_export(RACINE / "sauvegarde" / "ExportCSV-18-08-2026.csv").lignes
    b = cf.lire_export(RACINE / "sauvegarde" / "ExportCSV-19-08-2026.csv").lignes
...
def test_colonnes_decalees_donnent_autre():
    ...
    e = cf.lire_export(contenu, "ExportCSV-31-01-2026.csv")
