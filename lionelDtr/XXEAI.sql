create or replace PACKAGE XXEAI_INTERFACE_TOOLS_PKG AS
     ----------------------------------------------------------------------------------------------
     -- $ $
     --
     -- NOM               : XXEAI_INTERFACE_TOOLS_PKG.pks
     -- DESCRIPTION       : XXEAI_INTERFACE_TOOLS_PKG package spec : fonctions communes aux interface EAI
     ---
     ...
     --- ---------- --- -------------------------------------------------------------------------
     --  08/09/2014 BCO DPE20140079 - Evolution appel de fonction Banque pour l'EAI
     --  02/12/2014 BCO DPE20140088 - Evolution ajout des outils d'interface pour AP.
	   --  09/01/2015 BCO DPE20140114 - Trésorerie lot 2 - CAMT054 - Impayes Prélèvements clients.
     --  19/09/2016 HDE Modification pour HELIOS
     --  02/01/2016 MEG artf2071034 : ajout
     --------------------------------------------------------------------------------------------

     -----------------------------------------------------------------
     --  Variables globales
     -----------------------------------------------------------------
     gv_debug_mode VARCHAR2(1);
     ...
     PROCEDURE Get_Infos_Banque(pv_IBAN         IN VARCHAR2,
                                po_infos_banque IN OUT NOCOPY XXEAI_INFOS_BANQUE_OBJ);
	...
	PROCEDURE Get_Infos_Banque_Interne(pv_IBAN        IN VARCHAR2,
									   pv_Banque_Name IN OUT NOCOPY VARCHAR2);
	...
	PROCEDURE Get_Infos_Rejets_Sepa(pv_Code_Rejet        IN VARCHAR2,
									pv_Description_Rejet IN OUT NOCOPY VARCHAR2);
     ...
     --                  Type de la pièce et l'Origine de la pièce.
     --
     --  VALEUR RETOURNEE :
     --     Description       :  N/A
     --     Valeurs possibles :  N/A
     ...
     PROCEDURE Get_Params_FOLIO_GL(pv_FOLIO       IN VARCHAR2,
                                   po_cst_flux_gl IN OUT NOCOPY XXEAI_CST_FLUX_GL_OBJ);
     ...
     PROCEDURE Get_Societe_Projet(pv_projet  IN VARCHAR2,
                                  po_societe IN OUT NOCOPY XXEAI_SOCIETE_OBJ);
     ...
     --                  le type de dépense ou d'évènement.
     --
     --  PARAMETRES :
     --     po_dep_evt   Entrée      Objet regroupant le Code PR, le Code produit et le Compte Local
     --     po_societe   Sortie      Objet regroupant le Code erreur et le type de dépense/évènement.
     --
     --  VALEUR RETOURNEE :
     --     Description       :  N/A
     --     Valeurs possibles :  N/A
     ...
     PROCEDURE Get_Type_Depense_Evenement(po_dep_evt     IN XXEAI_IN_TYP_EVT_DEP_TYP,
                                          po_typ_evt_dep IN OUT NOCOPY XXEAI_TYP_EVT_DEP_OBJ);
     ...
     --          GL pour tous les comptes locaux et selon les règles de chaque folio.
     --
     --  PARAMETRES :
     ...
     --                      Code projet, Type de dépense ou d¿évènement, Quantité
     --     po_cle_comptable_cuf   Sortie      Objet regroupant Code Erreur, Société, Région, Compte local,
     --                      Compte Groupe, Complément groupe, Centre de coût, Code Affaire,
     --                      Code Activité, Flux, Réserve, Contexte du CUF, Id du projet, Type d¿évènement,
     --                        Type de dépense, Quantité, Id du projet à refacturer
     --
     --  VALEUR RETOURNEE :
     --     Description       :  N/A
     --     Valeurs possibles :  N/A
     ...
     PROCEDURE Get_Cle_Comptable_CUF(po_info_cle          IN XXEAI_INFO_CLE_TYP,
                                     po_cle_comptable_cuf IN OUT NOCOPY XXEAI_CLE_COMPTABLE_CUF_OBJ);

     -----------------------------------------------------------------
     --  Nom           : Get_Societe_Projet_Liste
     --  Description   : Fonction qui retourne dans un objet XXEAI_SOCIETE_LST
     --                  la liste de société du projet à partir d'une liste de code projet.
     --
     --  PARAMETRES :
     ...
     PROCEDURE Get_Societe_Projet_Liste(pt_projet      IN XXEAI_PROJET_TAB,
                                        po_societe_lst IN OUT NOCOPY XXEAI_SOCIETE_LST);

     -----------------------------------------------------------------
     --  FONCTIONNALITE 7 : DETERMINER LE TYPE DE DEPENSE/EVENEMENT EN LISTE
     --  Nom           : Get_Type_Depense_Evt_Liste
     --  Description   : Fonction qui retourne dans un objet XXEAI_TYP_EVT_DEP_OBJ
     --                  une liste de type de dépense ou d'évènement.
     --
     --  PARAMETRES :
     --     pt_in_dep_evt    Entrée      Objet regroupant la liste des codes PR, codes produits et compte local.
     --     po_typ_evt_dep   Sortie      Objet regroupant Code Erreur et la liste des codes erreurs et type de dépense/évènement
     --
     --  VALEUR RETOURNEE :
     --     Description       :  N/A
     --     Valeurs possibles :  N/A
     ...
     PROCEDURE Get_Type_Depense_Evt_Liste(pt_in_dep_evt  IN XXEAI_IN_TYP_EVT_DEP_TAB,
                                          po_typ_evt_dep IN OUT NOCOPY XXEAI_TYP_EVT_DEP_LST);
     ...
     --                      type de dépense/évènement et quantité.
     ...
     PROCEDURE Get_Cle_Comptable_CUF_Liste(pt_info_cle          IN XXEAI_INFO_CLE_TAB,
                                           po_cle_comptable_cuf IN OUT NOCOPY XXEAI_CLE_COMPTABLE_CUF_LST);

     ----------------------------------------------------------------------------------------------------
     --  FONCTIONNALITE 9 : DETERMINER L'IDENTIFIANT DE FACTURE/IDENTIFIANT DE LIGNE DE FACTURE.
     --  Nom           : Get_Invoice_Id
     --  Description   : Fonction qui génère dans un identifiant d'entête ou de ligne.
     --
     --  PARAMETRES :
     --     pv_Type            Entrée      valeur type ENTETE / LIGNE
     --
     --  VALEUR RETOURNEE : po_invoice_id
     --     Description       :  Si pv_Type = ENTETE alors génère l'identifiant de la facture
     --               Si pv_Type = LIGNE alors génère l'identifiant de la ligne de facture.
     --     Valeurs possibles :  Identifiant de l'entête/ligne de facture.
     ---------------------------------------------------------------------------------------------------
     PROCEDURE Get_Invoice_Id(pv_Type       IN VARCHAR2,
                              po_invoice_id IN OUT NOCOPY XXEAI_INVOICE_ID);
     ...
     --           pour l'entête de la facture selon les règles de chaque FOLIO.
     --
     --  PARAMETRES :
     ...
     --     Description       :  Retourne l'ensemble des informations permettant de compléter l'entête de la facture.
     ...
     PROCEDURE Get_Info_Invoice_Header(pt_in_header           IN XXEAI_IN_INVOICE_HEADER_TYP,
                                       po_info_invoice_header IN OUT NOCOPY XXEAI_INFO_INVOICE_HEADER_OBJ);
     ...
     PROCEDURE Get_Params_FOLIO_AP(pv_Folio       IN VARCHAR2,
                                   po_cst_flux_ap IN OUT NOCOPY XXEAI_CST_FLUX_AP_OBJ);

     ----------------------------------------------------------------------------------------------------
     --  FONCTIONNALITE 12 : DETERMINER LES INFORMATIONS POUR UNE LIGNE DE FACTURE
     --  Nom           : Get_Info_Invoice_Line
     --  Description   : Fonction qui retourne les informations pour les lignes de facture selon les règles de chaque Folio
     --
     --  PARAMETRES :
     ...
     PROCEDURE Get_Info_Invoice_Line(pt_info_line         IN XXEAI_IN_INVOICE_LINE_TYP,
                                     po_info_invoice_line IN OUT NOCOPY XXEAI_INFO_INVOICE_LINE_OBJ);
     ...
     --           les identifiants de clé comptable en fonction de la liste de paramètres en entrée.
     --
     --  PARAMETRES :
     ...
     PROCEDURE Get_Info_Invoice_Header_Liste(pt_info_header_tab IN XXEAI_IN_INVOICE_HEADER_TAB,
                                             po_info_header_lst IN OUT NOCOPY XXEAI_INFO_INVOICE_HEADER_LST);
     ...
     --           lignes de facture en fonction de la liste de paramètres en entrée.
     --
     --  PARAMETRES :
     ...
     PROCEDURE Get_Info_Invoice_Line_Liste(pt_info_line_tab IN XXEAI_IN_INVOICE_LINE_TAB,
                                           po_info_line_lst IN OUT NOCOPY XXEAI_INFO_INVOICE_LINE_LST);
     ...
     --           ou des nouveaux identifiants de lignes de factures suivant la liste de paramètres en entrée.
     --
     --  PARAMETRES :
     ...
     PROCEDURE Get_Invoice_Id_Liste(pt_type           IN XXEAI_TYPE_ENTETE_LIGNE_TAB,
                                    po_invoice_id_lst IN OUT NOCOPY XXEAI_INVOICE_ID_LST);

     -----------------------------------------------------------------
     --  FONCTIONNALITE 16 : DÉTERMINER LES TYPE ET NUMERO DE LIGNE ET LE TYPE DE MOUVEMENT AR
     --  Nom           : Get_type_num_ligne_mvt_AR
     --  Description   : Cette fonction sert à retourner le type de ligne, le numéro de ligne et le type de mouvement dune
     ...
     PROCEDURE Get_type_num_ligne_mvt_AR(pt_in_info_ligne     IN XXEAI_IN_LIGNE_MVT_AR_TYP,
                                         po_info_ligne_mvt_ar IN OUT NOCOPY XXEAI_TYPE_AR_OBJ);

     -----------------------------------------------------------------
     --  FONCTIONNALITE 17 : Déterminer la région à partir du DKCODE
     --  Nom           : Get_region_DKCODE
     --  Description   : Cette fonction servira à retourner la région en fonction du DKcode. Chaque Unité Opérationnelle aura un DKcode
     --                  (de type BxxxxxxxxK avec xxxxxxxx le code numérique et K le caractère de contrôle) et suivant le jeu de transcodification
     ...
     PROCEDURE Get_region_DKCODE(pv_DKcode IN VARCHAR2,
                                 po_info_region IN OUT NOCOPY XXEAI_TYPE_INFO_REGION_OBJ);
...
create or replace PACKAGE BODY       XXEAI_INTERFACE_TOOLS_PKG AS
     ...
     --- 09/01/2015 BCO DPE20140114 - TrÃ©sorerie lot 2 - CAMT054 - Impayes PrÃ©levements clients.
     --- 12/01/2015 TBO Modifications 002.3.8 et 002.4.11 suite recette fonctions GL
     --- 22/01/2015 BCO DPE20150006 - Outils d?interface AP pour l?EAI ? ProblÃ©matiques de TVA
     --- 11/02/2015 BCO Modifications de la recherche de la rÃ©gion dans la procÃ©dure Get_Info_Invoice_Line
     --- 25/03/2015 BCO DPE20150018 - TrÃ©sorerie(Diapason) pour Get_Cle_Comptable_CUF
     --- 25/03/2015 BCO DPE20150024 - Interdire les cas de refacturation si le type de piece est a contrepassation automatique.
     --- 26/03/2015 BCO DPE20150029 - Nouvelles regles de gestion pour la dÃ©termination du centre de gestion application DIAPASON.
     --- 08/04/2015 BCO DPE20150031 ? Gestion de la rÃ©gion pour le compte 585000 des Ã©critures venant de DIAPASON
     --- 10/04/2015 BCO DPE20150037 - Gestion du code partenaire pour le flux DIAPASON
     --- 13/04/2015 BCO DPE20150036 - Outils EAI AP - Modification RG dÃ©termination du Site pour les Fournisseurs Intra Groupe
     ...
     --- 29/05/2017 OBE - artf2159071 : SPE182 - Get_Info_Invoice_Header et Get_Info_Invoice_Line problÃšme REFAC - Defect Bout_en_Bout_Helios_Ref #697/699
     ...
     ---                - EDB075 Mapping SEGMENT8 et rÃšgle de validation croisÃ©e
     --- 27/06/2017 JJA - artf2182117 : SPE182 - EDB079 - Ajout rÃ©gion Get_Info_Invoice_Lines pour gÃ©rer la refacturation
     --- 18/07/2017 OBE artf2194552 : SPE182 - EDB081 - Gestions des Ã©critures de trÃ©sorerie
     --- 26/07/2017 ADE artf2202194 : SPE182 - compte de bilan rectification paramÃštre obligatoire
     --- 31/07/2017 YWA artf2204539 : SPE182 - inversion code banque et code BIC (swift_code) - Defect Beb #855
     --- 03/08/2017 YWA artf2206549 : SPE182 - Get_Cle_Comptable_CUF - contrÃŽles paramÃštres RÃ©gion et TÃ¢ches - defect BeB #900
     --- 29/08/2017 YWA artf2215513 : SPE182 - Gestion des Ã©critures de paie_Context MAT et ATTRIBUTE11 - EDB083
     --- 30/08/2017 YWA artf2216107 : SPE182 - Get_Info_Invoice_Line_Organisation de la dÃ©pense KO dans le cas REFAC - Defect Beb #1063
     --- 12/09/2017 OBE artf2222144 : SPE182 - Outils d'interface pour l'EAI - liÃ© Ã  modif sur SPE048 pour defect ORACLE_R12 #920
     --- 14/09/2017 OBE artf2221959 : SPE182 - SEGMENT7 retour arriÃšre - Defect Beb 1088
     --- 05/10/2017 OBE artf2237928 : SPE182 - Rattrapage contrepassation 11i - Defect Beb #1108
     --- 07/02/2018 JJA artf2361312 : SPE182 - Correction TrÃ©sorerie Infos banques
     --- 13/03/2018 OBE artf2408078 : DPE20180009 EDB093_SPE182_Code taxe pour les tÃ¢ches investissement
     --- 19/03/2018 OBE artf2408081 : DPE20180010 EDB097_SPE182_Gestion des PÃ©riodes FUTURES
     --- 27/03/2018 OBE artf2470550 : SPE182 - Correction code erreur Get_type_num_ligne_mvt_AR
   --- 04/02/2019 SFA artf3063732 : INC0283997 - Campagne rÃšglement - IBAN sur plusieurs fournisseurs
   --- 05/08/2019 SEL artf07290731 : INC0283997 - Campagne rÃšglement - IBAN sur plusieurs fournisseurs
   --- 17/09/2019 SFA artf07308610 :  INC0347680 - SPE182 - Interface EAI - Ajout des prÃ©fixes APPS
   --- 10/02/2021 NBO artf07438559 : INC0508700 - traitement de ces Ã©critures par la PFE a durÃ© plus de 3 heures
   --- 01/10/2021 ARA EDB290  DPE20210007 - Virements fournisseurs - Retirer le contrÃŽle sur les banques et agences fermÃ©s
   --- 02/11/2023 ARA EDB281 - DGE20210006 - Alimentation du segment5 de la clÃ© comptable
	--- 16/09/2025 BNZ EDB434 - Projet DEMAT - EDB434 - Interface entrante des factures clients dans AR - MAJ RG sur les comptes locaux
     --------------------------------------------------------------------------------------------
     ...
     --  Description   : ProcÃ©dure insÃ©rant un message dans la sortie standard
     --                  si on est en mode debug
     --
     --  PARAMETRES :
     --     pv_message   EntrÃ©e      Message a insÃ©rer
     --
     --  VALEUR RETOURNEE :
     --     N/A
     -----------------------------------------------------------------
     ...
     begin
       validate(p_param_value is not null, p_code_erreur, 'OAE021', 'Le parametre obligatoire ' || p_param_name || ' n''est pas renseignÃ©.');
     end not_null;
    ...
    AND closing_status in ('O','F') -- OBE artf2408081
     ...
     function is_numeric(p_value in varchar2) return boolean is
       v_num number;
     begin
       v_num := to_number(p_value);          -- attention : sÃ©parateur dÃ©cimal = .
       return true;
     exception
     ...
     begin
       ...
       if v_segment_info.validation_type != 'I' then
         put_debug_message('check_segment_value : seuls les jeux de valeurs indÃ©pendants sont pris en charge : ' || v_segment_info.flex_value_set_name);
         return false;
      end if;
      ...
      begin
        put_debug_message(lv_current_program_unit || ' 001 : Recherche livre de la sociÃ©tÃ© : ' || pv_segment1);
     ...
     -----------------------------------------------------------------
     -- FONCTIONNALITÃ 1 : INFORMATIONS BANCAIRE EN FONCTION DE L'IBAN
     --  Nom           : Get_Infos_Banque
     --  Description   : Fonction qui retourne dans un objet XXEAI_INFOS_BANQUE_OBJ
     --                  les informations de la banque
     --
     --  PARAMETRES :
     --     pv_IBAN          EntrÃ©e      IBAN
     --     po_infos_banque  Sortie      Objet regroupant le code banque,
     --                                  le nom de la banque et le code BIC
     --
     --  VALEUR RETOURNEE :
     ...
     BEGIN
       ...
       put_debug_message(lv_current_program_unit || ' 001 : DÃ©coupage de l''IBAN');
      ...
      /*//---------fin Ajout SFA -- pour artf3063732--------------------//*/
       ...
       put_debug_message(lv_current_program_unit || ' 003 : CrÃ©ation Objet');
    ...
    EXCEPTION
     ...
     -----------------------------------------------------------------
     -- FONCTIONNALITÃ 2 : DÃTERMINER LES VALEURS CONSTANTES DU FLUX
     --  Nom           : Get_Params_FOLIO_GL
     --  Description   : Fonction qui retourne dans un objet XXEAI_CST_FLUX_GL_OBJ
     --                  les valeurs constantes du flux pour les folio dÃ©terminÃ©.
     --
     --  PARAMETRES :
     --     pv_FOLIO         EntrÃ©e      FOLIO
     --     po_cst_flux_gl   Sortie      Objet regroupant le code erreur, code folio oracle,
     --                  statut, id Livre Comptable, Devise, Type d?Ã©criture,
     ...
     BEGIN
        put_debug_message(lv_current_program_unit || ' 000 : DÃ©but.');
     ...
     -----------------------------------------------------------------
     --  FonctionnalitÃ© 3 : DÃ©terminer la sociÃ©tÃ© du projet
     --  Nom           : Get_Societe_Projet
     --  Description   : Fonction qui retourne dans un objet XXEAI_SOCIETE_OBJ
     --                  les informations de la sociÃ©tÃ© du projet.
     --
     --  PARAMETRES :
     --     pv_projet    EntrÃ©e      projet
     --     po_societe   Sortie      Objet regroupant le code erreur et la sociÃ©tÃ© oracle du projet.
     --
     --  VALEUR RETOURNEE :
     --     Description       :  N/A
     --     Valeurs possibles :  N/A
     ...
     BEGIN
        ...
        put_debug_message(lv_current_program_unit || ' 001 : Recherche de la sociÃ©tÃ© du projet ' || v_project.project_id);
     ...
     --  Nom           : Get_Type_Depense_Evenement
     --  Description   : Fonction qui retourne dans un objet XXEAI_TYP_EVT_DEP_OBJ
     --                  le type de dÃ©pense ou d'Ã©venement.
     --
     --  PARAMETRES :
     --     po_dep_evt   EntrÃ©e      Objet regroupant le Code PR, le Code produit et le Compte Local
     --     po_societe   Sortie      Objet regroupant le Code erreur et le type de dÃ©pense/Ã©venement.
     --
     --  VALEUR RETOURNEE :
     --     Description       :  N/A
     --     Valeurs possibles :  N/A
     ...
     BEGIN
        ...
        validate(po_dep_evt.folio != 'MOD', vv_code_erreur, 'OAE016', 'Le folio MOD n''est pas pris en charge par cette fonction');  -- TODO : crÃ©er un code erreur dÃ©diÃ©
        ...
        put_debug_message(lv_current_program_unit || ' 002 : DÃ©termination du type de dÃ©pense/Ã©venement : ' || v_folio_info.MODE_DEP_EVT);
        ...
        -- RG_F04_02
        -- recherche du type de dÃ©pense / Ã©venement dans la transco qui convient
        if vv_type_depense_evt is null then
          put_debug_message(lv_current_program_unit || ' 002.1 : DÃ©termination de la transco PA');
          case
    ...
    --  Description   : recherche des infos du du compte local (segment3) passÃ© en paramÃštre
    ----------------------------------------------------------------------------------------
    ...
    BEGIN
        ...
        /*APPS.Dka_Tools_Pkg.get_flex_key_seg_value_set
            ...
            pv_legal_flex_key_flag       => 'N',   -- on considÃšre la ccf des organisations opÃ©rationnelles
            pn_segment_num               => 3,
   ...
   --- 10/02/2021 NBO artf07438559 : INC0508700 - traitement de ces Ã©critures par la PFE a durÃ© plus de 3 heures
    ...
    END get_account_info;
     ...
     -----------------------------------------------------------------
     -- FONCTIONNALITÃ 5 : DÃTERMINER LA CLÃ COMPTABLE ET LES ÃLÃMENTS DU CUF GL
     --  Nom           : Get_Cle_Comptable_CUF
     --  Description   : Fonction qui retourne dans un objet XXEAI_CLE_COMPTABLE_CUF_OBJ
     --                  les valeurs de chaque segment de la clÃ© comptable et les champs du CUF
     --          GL pour tous les comptes locaux et selon les regles de chaque folio.
     --
     --  PARAMETRES :
     --     po_info_cle            EntrÃ©e      Objet regroupant Folio AMONT, SociÃ©tÃ© de l?Ã©criture,
     --                      Segmentation (Appartenance), Compte local, Code Partenaire,
     --                      Code projet, Type de dÃ©pense ou d?Ã©venement, QuantitÃ©
     --     po_cle_comptable_cuf   Sortie      Objet regroupant Code Erreur, SociÃ©tÃ©, RÃ©gion, Compte local,
     --                      Compte Groupe, ComplÃ©ment groupe, Centre de cout, Code Affaire,
     --                      Code ActivitÃ©, Flux, RÃ©serve, Contexte du CUF, Id du projet, Type d?Ã©venement,
     --                        Type de dÃ©pense, QuantitÃ©, Id du projet a refacturer
     --
     --  VALEUR RETOURNEE :
     --     Description       :  N/A
     --     Valeurs possibles :  N/A
     ...
     BEGIN
       ...
       put_debug_message(lv_current_program_unit || ' 001 : VÃ©rification des valeurs obligatoires');
     ...
     --not_null('REGION',       po_info_cle.region,       vv_code_erreur); --YWA artf2206549
       ...
       -- VÃ©rification de la validitÃ© du folio
       put_debug_message(lv_current_program_unit || ' 002 : VÃ©rification du folio');
    ...
    END IF;
       ...
       if po_info_cle.tache_projet_finance is not null then
         put_debug_message(lv_current_program_unit || ' 002 : Recherche de la tÃ¢che et du projet finance : ' || po_info_cle.tache_projet_finance);
         ...
         -- sociÃ©tÃ© du projet
         put_debug_message(lv_current_program_unit || ' 003 : Recherche de la sociÃ©tÃ© du projet : ' || vv_project_info.segment1);
       ...
       -- Ledger_ID
       put_debug_message(lv_current_program_unit || ' 004 : Recherche livre de la sociÃ©tÃ© ' || po_info_cle.societe);
       ...
       -- VÃ©rification de la validitÃ© de la date comptable
       put_debug_message(lv_current_program_unit || ' 005 : Validation pÃ©riode comptable ouverte : ' || po_info_cle.DATE_COMPTABLE);
       validate(period_is_open(vo_result.ledger_id, po_info_cle.DATE_COMPTABLE), vv_code_erreur, 'OAE014');

       -- segment 1 : sociÃ©tÃ©
       put_debug_message(lv_current_program_unit || ' 006 : Validation sociÃ©tÃ© : ' ||  po_info_cle.societe);
        ...
        ELSE
            ...
            ELSE
                ...
                ELSE
                    ...
                    -- Recherche des infos du type de piÃšce
                    ...
                    --EDB281
					 ...
					 where ffvs.flex_value_set_id = ffv.flex_value_set_id
                       ...
                       END IF;*/ --EDB281 Lot2 activer ultÃ©riereument
       ...
       CASE
         ...
         WHEN substr(po_info_cle.compte_local, 1, 1) between '1' and '5' THEN
           ...
           -- segment4 : compte analytique => DÃ©terminÃ© en fonction du parametre 6 en interrogeant la table de transcodification MIGR_GROUP_ACCOUNT_CLASS_BILAN
         ...
         WHEN substr(po_info_cle.compte_local, 1, 1) in ('6', '7') THEN
           ...
           */
           ...
           if vv_societe_projet != po_info_cle.societe THEN
             vb_refac :=true;
             put_debug_message(lv_current_program_unit || ' 010 : Recherche de la tÃ¢che et du projet de refac : ' || po_info_cle.region || po_info_cle.societe);
             ...
             vv_task_refac_info := get_task(p_project_id => vv_project_refac_info.project_id, p_task_number => '1');     -- la tÃ¢che s'appelle toutjours 1
           ...
           if po_info_cle.compte_local like '6%' then
             put_debug_message(lv_current_program_unit || ' 011 : Recherche du type de dÃ©pense ' || po_info_cle.TYPE_DEP_EVT);
             ...
             -- Attribute12 : Type d'Ã©venement
             vo_result.attribute12 := NULL;

             -- Attribute13 : Type de dÃ©pense
             vo_result.attribute13 := vv_expenditure_type.expenditure_type;


           else        -- classe 7
             put_debug_message(lv_current_program_unit || ' 011 : Recherche du type d''Ã©venement ' || po_info_cle.TYPE_DEP_EVT);
           ...
           -- Attribute14 : QuantitÃ©
           put_debug_message(lv_current_program_unit || ' 013 : Validation quantitÃ© : ' || po_info_cle.quantite);
       ...
       IF vo_result.context3 = 'MAT' THEN --YWA  artf2215513
         ...
         -- Attribute18 : Id de la tÃ¢che a refacturer
	  ...
	  ELSE
       ...
       --> OBE artf2194552
         ...
         ELSE
           --DÃ©termination du code affaire sur le projet
           IF po_info_cle.TACHE_PROJET_FINANCE is not null THEN
             IF vb_refac THEN                                  -- OBE artf2171905
       ...
       -- Segment10 : RÃ©serve 1
       vo_result.segment10 := '0';

       -- Segment11 : RÃ©serve 2
       vo_result.segment11 := '0';

       -- Segment12 : RÃ©serve 3
       vo_result.segment12 := '0';


       --< OBE artf2194552
       IF vo_result.segment4 IS NULL THEN
        validate(vo_result.segment4 IS NOT NULL, vv_code_erreur, 'OAE008', 'Erreur : Segment4 n''a pas Ã©tÃ© dÃ©terminÃ©');
       END IF;
       --> OBE artf2194552
     ...
     -----------------------------------------------------------------
     --  FonctionnalitÃ© 6 : DÃ©terminer la sociÃ©tÃ© du projet en liste
     --  Nom           : Get_Societe_Projet_Liste
     --  Description   : Fonction qui retourne dans un objet XXEAI_SOCIETE_LST
     --                  la liste de sociÃ©tÃ© du projet a partir d'une liste de code projet.
     --
     --  PARAMETRES :
     --     pt_projet        EntrÃ©e      Objet regroupant la liste des codes projet.
     --     po_societe_lst   Sortie      Objet regroupant la liste des codes erreur
     --                  et des sociÃ©tÃ©s Oracle du projet.
     --
     --  VALEUR RETOURNEE :
     --     Description       :  N/A
     --     Valeurs possibles :  N/A
     ...
     BEGIN
       ...
       WHILE i is not null LOOP
         put_debug_message(lv_current_program_unit || ' 001.1 : Recherche de la sociÃ©tÃ© pour le projet : ' || pt_projet(i));
         Get_Societe_Projet(pt_projet(i), po_societe_obj);
         put_debug_message(lv_current_program_unit || ' 001.2 : Insertion de la sociÃ©tÃ© dans la collection des projets.');
         po_societe_tab.extend;
         po_societe_tab(po_societe_tab.last) := po_societe_obj;
         IF po_societe_obj.CODE_ERREUR != 'OAE000' THEN
           put_debug_message(lv_current_program_unit || ' 001.3 : Erreur dÃ©tectÃ©e dans l''exÃ©cution.');
    ...
    END Get_Societe_Projet_Liste;
     ...
     --  Nom           : Get_Type_Depense_Evt_Liste
     --  Description   : Fonction qui retourne dans un objet XXEAI_TYP_EVT_DEP_OBJ
     --                  une liste de type de dÃ©pense ou d'Ã©venement.
     --
     --  PARAMETRES :
     --     pt_in_dep_evt    EntrÃ©e      Objet regroupant la liste des codes PR, codes produits et compte local.
     --     po_typ_evt_dep   Sortie      Objet regroupant Code Erreur et la liste des codes erreurs et type de dÃ©pense/Ã©venement
     --
     --  VALEUR RETOURNEE :
     --     Description       :  N/A
     --     Valeurs possibles :  N/A
     ...
     BEGIN
       ...
       WHILE i is not null LOOP
         put_debug_message(lv_current_program_unit || ' 001.1 : Recherche du type d''Ã©venement pour le folio : ' || pt_in_dep_evt(i).folio);
         Get_Type_Depense_Evenement(pt_in_dep_evt(i), po_type_evt_obj);
         put_debug_message(lv_current_program_unit || ' 001.2 : Insertion du type d''Ã©venement dans la collection.');
    ...
    EXCEPTION
     ...
     -----------------------------------------------------------------
     -- FONCTIONNALITÃ 8 : DÃTERMINER LA CLÃ COMPTABLE ET LES ÃLÃMENTS DU CUF GL EN LISTE
     --  Nom           : Get_Cle_Comptable_CUF_Liste
     --  Description   : Fonction qui retourne dans un objet XXEAI_TYP_EVT_DEP_OBJ
     --                  les valeurs de chaque segment de la cl comptable et les champs du CUF
     --          GL pour tous les comptes locaux et selon les regles de chaque folio.
     --
     --  PARAMETRES :
     --     pt_info_cle            EntrÃ©e      Objet regroupant la liste des Folio Amont, sociÃ©tÃ© de l'Ã©criture,
     --                      segmentation, compte local, code partenaire, code projet,
     --                      type de dÃ©pense/Ã©venement et quantitÃ©.
     --     po_cle_comptable_cuf   Sortie      Objet regroupant Code Erreur et la liste des Code Erreur, SociÃ©tÃ©, RÃ©gion, Compte local,
     --                      Compte Groupe, ComplÃ©ment groupe, Centre de cout, Code Affaire,
     --                      Code ActivitÃ©, Flux, RÃ©serve, Contexte du CUF, Id du projet, Type d?Ã©venement,
     --                        Type de dÃ©pense, QuantitÃ©, Id du projet a refacturer
     --
     ...
     BEGIN
       ...
       WHILE i is not null LOOP
         put_debug_message(lv_current_program_unit || ' 001.1 : Recherche des infos de clÃ© comptable pour le folio : ' || pt_info_cle(i).folio);
         Get_Cle_Comptable_CUF(pt_info_cle(i), po_cle_comptable_cuf_obj);
         put_debug_message(lv_current_program_unit || ' 001.2 : Insertion du rÃ©sultat dans la collection.');
    ...
    EXCEPTION
     ...
     --  FONCTIONNALITE 9 : DETERMINER L'IDENTIFIANT DE FACTURE/IDENTIFIANT DE LIGNE DE FACTURE.
     --  Nom           : Get_Invoice_Id
     --  Description   : Fonction qui gÃ©nere dans un identifiant d'entete ou de ligne.
     --
     --  PARAMETRES :
     --     pv_Type            EntrÃ©e      valeur type ENTETE / LIGNE
     --
     --  VALEUR RETOURNEE : po_invoice_id
     --     Description       :  Si pv_Type = ENTETE alors gÃ©nere l'identifiant de la facture
     --               Si pv_Type = LIGNE alors gÃ©nere l'identifiant de la ligne de facture.
     ...
     BEGIN
        put_debug_message(lv_current_program_unit || ' 001 : VÃ©rifier l''existence des champs obligatoires.');
        not_null('pv_Type', pv_Type, vv_code_erreur);

        put_debug_message(lv_current_program_unit || ' 002 : VÃ©rification du type');
        BEGIN
          IF pv_Type = 'ENTETE' THEN
            put_debug_message(lv_current_program_unit || ' 003 : Type d''identifiant gÃ©nÃ©rÃ© "' || pv_Type ||'"');
        ...
        put_debug_message(lv_current_program_unit || ' 004 : CrÃ©ation Objet');
     ...
     --  Nom           : Get_Info_Invoice_Header
     --  Description   : Fonction qui retourne les informations du fournisseur, le type (encaissement/
     --         dÃ©caissement), le nom du groupe de paiement et l'identifiant de la clÃ© comptable
     --           pour l'entete de la facture selon les regles de chaque FOLIO.
     --
     --  PARAMETRES :
     --     pt_in_header    EntrÃ©e      Objet contenant la liste des informations suivantes:
     --                     Code Folio Oracle
     --                  Identifiant Fournisseur Externe
     --                  SociÃ©tÃ©
     --                    Appartenance
     ...
     --                    Compte Local
     --  VALEUR RETOURNEE : po_info_invoice_header
     --     Description       :  Retourne l'ensemble des informations permettant de complÃ©ter l'entete de la facture.
     --     Valeurs possibles :  Identifiant du Fournisseur
     --             Identifiant du site fournisseur
     --               Type (Encaissement / DÃ©caissement)
     --               Nom du groupe de paiement
     --               Identifiant de la clÃ© comptable
     --               Type de document
     ---------------------------------------------------------------------------------------------------
     PROCEDURE Get_Info_Invoice_Header(pt_in_header           IN XXEAI_IN_INVOICE_HEADER_TYP,
     ...
     BEGIN
       ...
       if v_params_folio.TYPE_FOURNISSEUR = 'ORACLE' then
         if length(pt_in_header.ID_FRS_EXT) = 4 then
           ...
           if cur_supplier_groupe%NOTFOUND then
             put_debug_message(lv_current_program_unit || ' 004 : Fournisseur Groupe non trouvÃ© ' || pt_in_header.ID_FRS_EXT);
         ...
         if cur_supplier_oracle%NOTFOUND then
           put_debug_message(lv_current_program_unit || ' 004 : Fournisseur non trouvÃ© ' || pt_in_header.ID_FRS_EXT);
         ELSE
           vb_fournisseur := TRUE;
           --validate(FALSE, vv_code_erreur, 'OAE024', lv_current_program_unit || ' 004 : Fournisseur non trouvÃ© ' || pt_in_header.ID_FRS_EXT);
         end if;
         ...
         put_debug_message(lv_current_program_unit || ' 004 : recherche de la tÃ¢che du projet : ' || pt_in_header.TACHE_PROJET_FINANCE);
       ...
       EXCEPTION
          WHEN OTHERS THEN
          Validate(FALSE, vv_code_erreur, 'OAE044',lv_current_program_unit||' 005 : UO non trouvÃ©e' );
       END;
     ...
     --  FONCTIONNALITE 11 : DETERMINER LES VALEURS CONSTANTES DU FLUX AP
     --  Nom           : Get_Params_FOLIO_AP
     --  Description   : Fonction qui retourne l'ensemble des valeurs constantes du flux pour un folio dÃ©terminÃ©.
     --
     --  PARAMETRES :
     --     pv_Folio    EntrÃ©e      Code Folio Amont
     --  VALEUR RETOURNEE : po_cst_flux_ap
     --     Description       :  Retourne l'ensemble des constantes du flux.
     --     Valeurs possibles :  Code Erreur
     --             Code devise de la facture
     --               Code folio Oracle
     --               CatÃ©gorie de la structure du CUF dÃ©pendant de la localisation
     --               Origine de la facture
     --               Type de taux dÃ©rivÃ©s
     --               Taux dÃ©rivÃ©s
     --               Devise de reglement
     --               Nom de l'application permettant de dÃ©terminer le Type de document.
     --               Identifiant de l'organisation.
     ---------------------------------------------------------------------------------------------------
     PROCEDURE Get_Params_FOLIO_AP(pv_Folio       IN VARCHAR2,
     ...
     --
     --  PARAMETRES :
     --     pt_info_line    EntrÃ©e      Objet contenant la liste des informations suivantes:
     --                     Code FOLIO Oracle
     --                     Code du Projet / Appartenance
     --                     Nature1
     --                     Nature2
     ...
     --                Type de ligne de ventilation
     --                Identifiant de la clÃ© comptable
     --                CatÃ©gorie de structure
     --                Identifiant du projet de refacturation
     --                Identifiant du projet
     --                Identifiant de tÃ¢che
     --                Type de dÃ©pense
     --                Identifiant de l?organisation de dÃ©pense
     --                Code de TAX
     ---------------------------------------------------------------------------------------------------
     PROCEDURE Get_Info_Invoice_Line(pt_info_line         IN XXEAI_IN_INVOICE_LINE_TYP,
        ...
        vn_ledger_id                 NUMBER;
        vv_code_societe              VARCHAR2(40);
        vv_code_region               VARCHAR2(40); -- Region passÃ©e en paramÃštre
    ...
    vv_caractere_tax             VARCHAR2(1); -- OBE artf2408078
     ...
     BEGIN
       ...
       --Type de ligne de ventilation.  Si le compte local est prÃ©sent dans le jeu de valeurs DKA_LOCAL_ACCOUNT_DIVERS (cf. Â§5.1.1), le type de ligne de ventilation aura la valeur 'MISCELLANEOUS'.
       ...
       put_debug_message(lv_current_program_unit || ' 003 : Recherche de la tÃ¢che et du projet finance : ' || pt_info_line.TACHE_PROJET);
       ...
       -- sociÃ©tÃ© du projet
       put_debug_message(lv_current_program_unit || ' 004 : Recherche de la sociÃ©tÃ© du projet : ' || vv_project_info.segment1);
       ...
       put_debug_message(lv_current_program_unit || ' 005 : Recherche de la rÃ©gion du projet : ' || vv_project_info.segment1);
       ...
       -- Identifiant de la clÃ© comptable Si le type de ligne est Divers (MISCELLANEOUS), la clÃ© comptable aura la valeur comme prÃ©cisÃ© ci- dessous.
       -- Si le type de ligne est diffÃ©rent de Divers (Montant HT, TVA), le champ sera vide.
       if v_result.LINE_TYPE_LOOKUP_CODE = 'MISCELLANEOUS' then
         put_debug_message(lv_current_program_unit || ' 006 : recherche du livre de la sociÃ©tÃ© ' || pt_info_line.SOCIETE);
         ...
         put_debug_message(lv_current_program_unit || ' 006 : Identifiant de la clÃ© comptable');
       ...
       IF v_result.LINE_TYPE_LOOKUP_CODE = 'ITEM' then
         ...
         -- type de dÃ©pense
         put_debug_message(lv_current_program_unit || ' 008 : Recherche du type de dÃ©pense : ' ||
       ...
       -- Si le type de ligne a la valeur TAX alors le champ TAX_CODE sera dÃ©terminÃ©. On concatenera les informations suivantes :
       --   DED_ + le taux avec un point en sÃ©parateur +(Si le rÃ©gime fiscal est CRE/M alors E si le rÃ©gime fiscal est  DEB/M alors D).
       if v_result.LINE_TYPE_LOOKUP_CODE = 'TAX' then
         put_debug_message(lv_current_program_unit || ' 009 : DÃ©termination du code de taxe : ' || pt_info_line.TAX_RATE ||', ' || pt_info_line.REGIME_TVA);
      ...
      -- CatÃ©gorie de structure. Ce champ n'est pas renseignÃ© dans la table d'interface AP.
       v_result.ATTRIBUTE_CATEGORY := null;

       put_debug_message(lv_current_program_unit || ' 010 : CrÃ©ation Objet');
     ...
     --  Nom           : Get_Info_Invoice_Header_Liste
     --  Description   : Fonction qui retourne la liste de valeurs des informations fournisseurs,
     --           les types (encaissement/dÃ©caissement), les noms de groupe de paiement et
     --           les identifiants de clÃ© comptable en fonction de la liste de parametres en entrÃ©e.
     --
     --  PARAMETRES :
     --     pt_info_header_tab   EntrÃ©e      Objet contenant un tableau d'objet
     --     po_info_header_lst   Sortie      Objet contenant une liste d'objet XXEAI_INFO_INVOICE_HEADER_TAB
     --
     --  VALEUR RETOURNEE :
     --     Description       :  N/A
    ...
    EXCEPTION
     ...
     --  Nom           : Get_Info_Invoice_Line_Liste
     --  Description   : Fonction qui retourne la liste de valeurs des informations pour les
     --           lignes de facture en fonction de la liste de parametres en entrÃ©e.
     --
     --  PARAMETRES :
     --     pt_info_line_tab     EntrÃ©e        Objet contenant un tableau d'objet
     --     po_info_line_lst     Sortie        Objet contenant une liste d'objet XXEAI_INFO_INVOICE_LINE_LST
     --
     --  VALEUR RETOURNEE :
     --     Description       :  N/A
    ...
    EXCEPTION
     ...
     --  Nom           : Get_Invoice_Id_Liste
     --  Description   : Fonction qui retourne la liste de valeurs des nouveaux identifiants de factures
     --           ou des nouveaux identifiants de lignes de factures suivant la liste de parametres en entrÃ©e.
     --
     --  PARAMETRES :
     --     pt_type         EntrÃ©e        Objet contenant un tableau d'objet
     --     po_invoice_id_lst     Sortie        Objet contenant une liste d'objet XXEAI_INVOICE_ID_LST
     --
     --  VALEUR RETOURNEE :
     --     Description       :  N/A
     ...
     BEGIN
       ...
       WHILE i is not null LOOP
         put_debug_message(lv_current_program_unit || ' 001.1 : RÃ©cupÃ©ration de l''identifiant : ' || pt_type(i));
     ...
     -----------------------------------------------------------------
     --  FONCTIONNALITE 16 : DÃTERMINER LES TYPE ET NUMERO DE LIGNE ET LE TYPE DE MOUVEMENT AR
     --  Nom           : Get_type_num_ligne_mvt_AR
     --  Description   : Cette fonction sert a retourner le type de ligne, le numÃ©ro de ligne et le type de mouvement d'une
     --                  facture AR en fonction du compte local, du sous compte, du signe, du dÃ©bit/crÃ©dit et du montant
     --                  fournis par les amonts.
     --
     --  PARAMETRES :
     --     pt_in_info_ligne         EntrÃ©e        Objet contenant le compte local, le sous-compte, le signe, DÃ©bit/crÃ©dit, et le montant
     --     po_info_ligne_mvt_ar     Sortie        Objet contenant le code erreur, le type de ligne, le numÃ©ro de ligne et le type de mouvement
     --
     --  VALEUR RETOURNEE :
     --     Description       :  N/A
     --     Valeurs possibles :  N/A
     ...
     PROCEDURE Get_type_num_ligne_mvt_AR(pt_in_info_ligne     IN XXEAI_IN_LIGNE_MVT_AR_TYP,
        ...
        vn_line_num       NUMBER(15);           -- NumÃ©ro de ligne
        vv_type_mvt       VARCHAR2(50);         -- Type de mouvement
		v_flex_value_type		cur_flex_value%rowtype;	--BNZ
     BEGIN
        put_debug_message(lv_current_program_unit || ' 001 : DÃ©but');
        not_null('COMPTE_LOCAL', pt_in_info_ligne.COMPTE_LOCAL, vv_code_erreur);
--> EDB434 BNZ

		v_flex_value_type := get_flex_value('DAOPCCF_LOCAL', pt_in_info_ligne.COMPTE_LOCAL);
        -- dÃ©termination du LINE_TYPE :
		if v_flex_value_type.ATTRIBUTE12 = 'ACC_CLIENT' and pt_in_info_ligne.SOUS_COMPTE is not null then
          vv_line_type := 'CLIENT';
        elsif v_flex_value_type.ATTRIBUTE12 = 'ACC_GESTION'  THEN
          vv_line_type := 'PRODUIT';
        elsif v_flex_value_type.ATTRIBUTE12 = 'ACC_PRODUIT' THEN
          vv_line_type := 'GESTION';
        elsif v_flex_value_type.ATTRIBUTE12 = 'ACC_SURTAXE' THEN
          vv_line_type := 'SURTAXE';
		elsif v_flex_value_type.ATTRIBUTE12 = 'ACC_NC' THEN
		    validate(FALSE, vv_code_erreur, 'OAE046', 'Le compte local utilisÃ© ne fait pas partie des comptes concernÃ©s. Compte local=' || pt_in_info_ligne.COMPTE_LOCAL);
        else
          validate(FALSE, vv_code_erreur, 'OAE038', 'Impossible de dÃ©terminer le type de ligne AR. Compte local=' || pt_in_info_ligne.COMPTE_LOCAL); -- OBE artf2470550
        end if;
...
*/
        ...
        if vv_line_type = 'CLIENT' THEN
          case
            ...
            when pt_in_info_ligne.SIGNE='-' and pt_in_info_ligne.DEBIT_CREDIT='C'                                  then
              ...
              validate(FALSE, vv_code_erreur, 'OAE039', 'Impossible de dÃ©terminer le type de mouvement AR. SIGNE=' || pt_in_info_ligne.SIGNE);
          end case;
        else
    ...
    END Get_type_num_ligne_mvt_AR;
     ...
     -----------------------------------------------------------------
     --  FONCTIONNALITE 17 : DÃ©terminer la rÃ©gion a partir du DKCODE
     --  Nom           : Get_region_DKCODE
     --  Description   : Cette fonction servira a retourner la rÃ©gion en fonction du DKcode. Chaque UnitÃ© OpÃ©rationnelle aura un DKcode
     --                  (de type BxxxxxxxxK avec xxxxxxxx le code numÃ©rique et K le caractere de contrÃŽle) et suivant le jeu de transcodification
     --                  DKA_DKCODE_UO (DKCode <> UO_DALKIA)  on en dÃ©duira la rÃ©gion
     --
     --  PARAMETRES :
     --     pv_DKcode                EntrÃ©e        DKcode
     --     po_info_region           Sortie        Objet contenant le code erreur, et la rÃ©gion
     --
     --  VALEUR RETOURNEE :
     --     Description       :  N/A
     --     Valeurs possibles :  N/A
     ...
     -----------------------------------------------------------------
     -- FONCTIONNALITÃ 18 : INFORMATIONS BANCAIRE EN FONCTION DE L'IBAN SUR DES COMPTES DE BANQUES INTERNES (BANQUE DALKIA)
     --  Nom           : Get_Infos_Banque_Interne
     --  Description   : Fonction qui retourne le nom de la banque interne DALKIA
     --
     --  PARAMETRES :
     ...
     -----------------------------------------------------------------
     -- FONCTIONNALITÃ 19 : INFORMATIONS SUR LES CODES REJETS SUR LES PRELEVEMENTS CLIENTS (ETAT CAMT054)
     --  Nom           : Get_Infos_Rejets_Sepa
     --  Description   : Fonction qui retourne le nom de la banque interne DALKIA
     --
     --  PARAMETRES :
...
END XXEAI_INTERFACE_TOOLS_PKG; -- body du packag
END XXEAI_INTERFACE_TOOLS_PKG; -- Spécification du packag
