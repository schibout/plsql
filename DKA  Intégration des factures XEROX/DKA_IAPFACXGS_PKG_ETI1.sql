...
create or replace PACKAGE BODY      dka_iapfacxgs_pkg AS
  ...
  --------------------------------------------------------------------------------
  --- 2017/05/09 NBO artf2150386 - INT047 - Factures aÃ?ÃÂ aÃ?ÃÂ  recycler - Defect Bout_en_Bout_Helios_Ref #648Ac
  ...
  --- 2020/02/18 MTIA : INC0402570 : Initialisation variable pour eviter division aÃ?ÃÂ aÃ?ÃÂ  zero
  ...
  --- 2020/10/21 MTIA Correction sur Ã?ÃÂ©cart sur ligne avec projet de refacturation
  ...
  --- 2023/07/21 IBA  EDB355  INTERFACE DES FACTURES XEROX ÃÂ¢ÃÂ¿ÃÂ¿ GESTION DES CODES TVA ÃÂ¢ÃÂ¿ÃÂ¿ICÃÂ¢ÃÂ¿ÃÂ¿ ÃÂ¢ÃÂ¿ÃÂ¿ EVOLUTION REGLE DE GESTION CAS 2
  --- 2023/03/29 BNZ  EDB376  Rapprochement automatique de lÃÂ¿interface Xerox
  ...
  --- 2025/03/04 KHH  KAFI-1039 ;Recette KAFI-838 - IntÃ©gration XEROX  : si Cd SSTR le type est de 0003_SSTRT.
  --------------------------------------------------------------------------------
  ...
  -----------------------------------------------------------------
  --  Nom           : get_prefix_from_filename
  --  Description   : DéŽ¥rmine le préŠ©xe du nom de fichier
  -----------------------------------------------------------------
  ...
  BEGIN
    ...
    IF vv_prefix IS NULL THEN
      vv_prefix := upper(substr(trim(pv_nom_fichier), 1, 3));
    END IF;
  ...
  -----------------------------------------------------------------
  --  Nom           : get_source_from_filename
  --  Description   : determine la source AP partir du prefixe du nom
  --                  de fichier  dans DKA_IAPFACXGS_INTERFACE.NOM_FICHIER
  -----------------------------------------------------------------
  ...
  BEGIN
    ...
    CASE vv_prefix
      ...
      ELSE
        RETURN 'SCAN_XGS';
    END CASE;
  END get_source_from_filename;  --sho

  -----------------------------------------------------------------
  --  Nom           : get_folio_from_filename
  --  Description   : DéŽ¥rmine le folio à °artir du préŠ©xe du nom
  --                  de fichier pré³¥nt dans DKA_IAPFACXGS_INTERFACE.NOM_FICHIER
  -----------------------------------------------------------------
  FUNCTION get_folio_from_filename(pv_nom_fichier IN VARCHAR2)
    RETURN ap_invoices_interface.attribute9%TYPE IS
    vv_prefix VARCHAR2(30);
  BEGIN
    vv_prefix := get_prefix_from_filename(pv_nom_fichier);
...
--> EDB376
  ...
  */
  FUNCTION insert_header(pv_invoice_num                IN ap_invoices_interface.invoice_num%TYPE,
                         ...
                         pv_source                     IN ap_invoices_interface.source%TYPE,
  ...
  */
  PROCEDURE child_import(pv_errbuf OUT VARCHAR2, pn_retcode OUT NUMBER) IS
    ...
    CURSOR cur_error IS
      select invoice_id, invoice_num
       ...
       where aii.vendor_num like '%DOUBLON'
         ...
         AND aii.source IN ('SCAN_XGS','DEMAT') --sho
         and NOT EXISTS
       (select 'x'
    ...
    CURSOR cur_ap_interface IS
      SELECT AII.INVOICE_ID,
             ...
             AII.rowid rid
        FROM AP_INVOICES_INTERFACE AII
       WHERE AII.SOURCE IN ('SCAN_XGS','DEMAT') --sho
    ...
    vv_interface_origine           VARCHAR2(30);
    vv_group_key                   VARCHAR2(240);
    vv_group_source                ap_invoices_interface.source%TYPE;
    ...
    --MTIA : INC0402570 : Creation variable pour eviter division aÃ?ÃÂ aÃ?ÃÂ  zero
  ...
  -- SELO EDB235B FIN
    ...
    CURSOR cur_factures_ec IS
      SELECT aii.*
        FROM ap_invoices_interface aii
       WHERE aii.source IN ('SCAN_XGS','DEMAT')
    ...
    FOR rec_fact in cur_factures LOOP
      BEGIN
        ...
        IF vd_gl_date IS NULL THEN
          ...
          END IF;
        END IF;
        -- DéŽ¥rmination de la source / folio à °artir du prefixe du fichier
        vv_source := get_source_from_filename(rec_fact.nom_fichier); --shoussai
        vv_attribute9 := get_folio_from_filename(rec_fact.nom_fichier);
        ...
        ELSE
          vv_attribute14 := NULL;
        END IF;
        -- rg07 - Modification pour IVALUA / DEMAT --sho
        IF vv_attribute9 IN ('XGS') THEN
          BEGIN
            SELECT fdsa.category_code
             ...
             WHERE fdsa.category_code = rec_fact.code_societe || '_SCAN_XGS'
               ...
               AND trunc(vd_gl_date) BETWEEN trunc(fdsa.start_date)
                   AND trunc(nvl(fdsa.end_date, GREATEST(SYSDATE, vd_gl_date)));
          EXCEPTION
            WHEN OTHERS THEN
        ...
        ELSIF vv_attribute9 = 'DSP' THEN
          ...
          EXCEPTION
            WHEN OTHERS THEN
              ...
              vv_doc_category_code := NULL;
          END;
	    ELSIF vv_attribute9 IN ('OCR', 'DMT') THEN --sho

		  BEGIN
			SELECT fdsa.category_code
			  INTO vv_doc_category_code
			  FROM fnd_doc_sequence_assignments fdsa
			 WHERE fdsa.category_code = rec_fact.code_societe || '_DEMAT'
			   AND fdsa.method_code = 'A'
			   AND fdsa.set_of_books_id = gn_ledger_id
			   AND trunc(vd_gl_date) BETWEEN trunc(fdsa.start_date)
				   AND trunc(nvl(fdsa.end_date, GREATEST(SYSDATE, vd_gl_date)));
        ...
        IF vn_type_facture = 3 THEN       --- KAFI-1039 ;Recette KAFI-838 - IntÃ©gration XEROX
  ...
  Dka_Tools_Pkg.Get_Parameter(cv_program_code,
    ...
    END IF;
     ...
     dka_Tools_Pkg.put_log_message('vn_dummy2 : ' ||
        ...
        vv_group_key := vv_source || '|' || vv_group_id;
        IF NOT va_group_id.exists(vv_group_key) THEN
          va_group_id(vv_group_key) := vv_hold_name;
        END IF;
      ...
      BEGIN
            ...
            ELSIF nvl(rec_fact.type_tva, 'DE') IN ('DE', 'EX') THEN  --EDB325B enlevÃ?ÃÂ© 'IC
              vv_name := 'DED_' || substr(rec_fact.taux_tva, 1, 2) || '.' ||
  ...
  dka_Tools_Pkg.put_log_message('Facture Type :  ' || vv_type_lig );
      ...
      if v_count > 1 then
      dka_Tools_Pkg.put_log_message('==== > Commande avec plusieur Facture  NÃÂ° :  ' || v_count || 'Factures' );
         begin
         select Mht_dispo
         into v_Mht_dispo
...
/*      IF rec_fact.type_tva = 'IC' and l_cas2 = 1 then
      ...
      -- SELO EDB235B FIN
              ...
              IF (((rec_fact.type_piece = '0001') and
                ...
                --MTIA : INC0402570 : Initialisation variable pour eviter division aÃ?ÃÂ aÃ?ÃÂ  zero
...
/*      IF rec_fact.type_tva = 'IC' and l_cas2 = 1 then
  ...
  dka_Tools_Pkg.put_log_message('Facture Type :  ' || vv_type_lig );
  dka_Tools_Pkg.put_log_message('---------------- > Facture NÃÂ°  :  ' || rec_fact.num_fact );

         select count(*) into v_count from dka_iapfacxgs_interface where no_commande = rec_line.num_cde and flag_trt is null;
      if v_count > 1 then
      dka_Tools_Pkg.put_log_message('====== > Commande avec plusieur Facture  NÃÂ° :  ' || v_count );
         begin
         select Mht_dispo
         into v_Mht_dispo
         from DKA_IAPFACXGS_MTDISP where no_commande = rec_line.num_cde and org_id = gn_org_id ;
  ...
  ---FIN IBA EDB349>
    ...
    FOR head_facture IN cur_factures_ec LOOP
     ...
     -- IF ABS(vn_ecart_facture) <= vn_ecart_minime_facture AND
       ...
       THEN
        ...
        dka_Tools_Pkg.put_log_message('Il faut inserer une ligne pour la Commande noaÃ?ÃÂ°' ||
                                      head_facture.attribute13 ||
                                      ' et Invoice_Id noaÃ?ÃÂ°' ||
                                      vn_invoice_id || ' de ' ||
                                      vn_ecart_facture || '?');
        --MTIA Correction sur Ã?ÃÂ©cart sur ligne avec projet de refacturation
        ---------------------------------------------------------
    ...
    BEGIN
      ...
      --- 2017/05/09 NBO artf2150386 - INT047 - Factures aÃ?ÃÂ aÃ?ÃÂ  recycler - Defect Bout_en_Bout_Helios_Ref #648
      dka_Tools_Pkg.put_log_message('Recyclage des factures');

      FOR facture_recyclee in (select distinct group_id, source
                                 from ap_invoices_interface aii
                                where aii.status = 'REJECTED'
                                  and source in ('SCAN_XGS','DEMAT')
                                  and org_id = gn_org_id) LOOP
        -- Les valeurs de group_id et hold_name sont maintenant determinees.
        -- On les met dans un tableau pour les lancements du traitement
        -- standard d'import des commandes
        vv_group_key := facture_recyclee.source || '|' || facture_recyclee.group_id;
        IF NOT va_group_id.exists(vv_group_key) THEN
          vv_hold_name := null;
      ...
      vv_group_key := va_group_id.first;
      WHILE vv_group_key IS NOT NULL LOOP

        vv_group_source := substr(vv_group_key, 1, instr(vv_group_key, '|') - 1);
        vv_group_id     := substr(vv_group_key, instr(vv_group_key, '|') + 1);
        --sho
        IF vv_group_source = 'DEMAT' THEN
          vv_errbuf  := '';
          vn_retcode := 0;
          ...
          IF vn_retcode != 0 THEN
            dka_Tools_Pkg.put_log_message('Erreur lors de l''ajout des lignes de TVA Autoliquidee Origine OCR / Group_id: DEMAT / ' ||
                                          vv_group_id || ' : ' || vv_errbuf);
          ELSE
            dka_Tools_Pkg.put_log_message('Ajout des lignes de TVA Autoliquidee avec succes : Origine OCR / Group_id: DEMAT / ' ||
                                          vv_group_id || ' : ' || vv_errbuf);
          END IF;
          ...
          IF vn_retcode != 0 THEN
            dka_Tools_Pkg.put_log_message('Erreur lors de l''ajout des lignes de TVA Autoliquidee Origine DMT / Group_id: DEMAT / ' ||
                                          vv_group_id || ' : ' || vv_errbuf);
          ELSE
            dka_Tools_Pkg.put_log_message('Ajout des lignes de TVA Autoliquidee avec succes : Origine DMT / Group_id: DEMAT / ' ||
                                          vv_group_id || ' : ' || vv_errbuf);
          END IF;
        ELSE
          vv_errbuf  := '';
        ...
        vv_group_key := va_group_id.next(vv_group_key);
      END LOOP;
      ...
      BEGIN
        vv_group_key := va_group_id.first;
        WHILE vv_group_key IS NOT NULL LOOP
          vv_hold_name := va_group_id(vv_group_key);
          vv_group_source := substr(vv_group_key, 1, instr(vv_group_key, '|') - 1);
          vv_group_id := substr(vv_group_key, instr(vv_group_key, '|') + 1);
  ...
  BEGIN
    ...
    FOR rec_fact in cur_factures LOOP
      ...
      BEGIN
        ...
        --- 2018/08/27 NBO artf2877778 : DPE20180026 - EDB094 - Controle doublon facture AP sur site different
        -- Verifie si il y a un doublon facture : maÃ?ÃÂ aÃ?ÃÂªme numero / maÃ?ÃÂ aÃ?ÃÂªme montant / maÃ?ÃÂ aÃ?ÃÂªme date / maÃ?ÃÂ aÃ?ÃÂªme region / mais site different
        SELECT count(*)
          INTO vn_count_doublon
          FROM ap_invoices_all api, ap_suppliers pv, ap_supplier_sites pvs
         WHERE 1 = 1 --api.org_id = pn_org_id 30/04/2020 AAM artf07219745 enlever le critore unite operationnelle lors du controle de doublon
 ...
 ------------------------------------------------------------------------------------
  ...
  --  Description   : point d'entrÃ?ÃÂ©e pour l'insertion des donnÃ?ÃÂ©es de la DKA_IAPFACDSP_TMP dans
  --                  les tables    DKA.DKA_IAPFACXGS_INTERFACE, XXGE.XXGE_IAPFACXGS_REPORTING_ALL,
  --                  XXGE.XXGE_IAPFACXGS_INT_HIST_HEADS et XXGE.XXGE_IAPFACXGS_INT_HIST_LINES.
  --
  --  PARAMETRES    :
  ...
  PROCEDURE populate_dsp_tables(pv_errbuf  OUT VARCHAR2,
    ...
    -- timestamp pour la mesure du temps d'exÃ?ÃÂ©cution des fonctions/procÃ?ÃÂ©dures
    vd_func_starttime TIMESTAMP;
    -- timestamp pour le temps d'exÃ?ÃÂ©cution total
    vd_total_starttime TIMESTAMP;
  ...
  BEGIN
    ...
    -- dÃ?ÃÂ©marrage du programme et initialisation du timestamp total
    vd_total_starttime := LOCALTIMESTAMP;
    ...
    dka_Tools_Pkg.put_log_message('DÃ?ÃÂ©marrage du traitement');
    ...
    LOOP
      ...
      --CrÃ?ÃÂ©ation enreg DKA_IAPFACXGS_INTERFACE
      INSERT INTO DKA_IAPFACXGS_INTERFACE
        (TYPE,
         CODE_SOCIETE,
         CODE_DIVISION,
      ...
      --CrÃ?ÃÂ©ation enreg DKA_IAPFACXGS_REPORTING_ALL
      INSERT INTO DKA_IAPFACXGS_REPORTING_ALL
        (TYPE,
         CODE_SOCIETE,
         CODE_DIVISION,
    ...
    dka_Tools_Pkg.put_log_message('DKA_IAPFACDSP_TMP vers DKA_IAPFACXGS_INTERFACE et XXGE_IAPFACXGS_REPORTING_ALL, ' ||
                                  vn_interface || ' lignes insÃ?ÃÂ©rÃ?ÃÂ©es.');
    ...
    dka_Tools_Pkg.put_log_message('DKA_IAPFACDSP_TMP vers DKA_IAPFACXGS_HIST_LINES, ' ||
                                  vn_hist_lines || ' lignes insÃ?ÃÂ©rÃ?ÃÂ©es.');
    dka_Tools_Pkg.put_log_message('DKA_IAPFACDSP_TMP vers DKA_IAPFACXGS_HIST_HEADS, ' ||
                                  vn_hist_heads || ' lignes insÃ?ÃÂ©rÃ?ÃÂ©es.');

    dka_Tools_Pkg.put_log_message('temps Ã?ÃÂ©coulÃ?ÃÂ© [' ||
                                  TO_CHAR(LOCALTIMESTAMP -
                                          vd_func_starttime,
                                          'DD-MON-YYYY HH24:MI:SSxFF TZH:TZM') || ']');

    dka_Tools_Pkg.put_log_message('Fin du traitement, ' || SQL%ROWCOUNT ||
                                  ' lignes insÃ?ÃÂ©rÃ?ÃÂ©es.');

    dka_Tools_Pkg.put_log_message('temps total Ã?ÃÂ©coulÃ?ÃÂ© : [' ||
                                  TO_CHAR(LOCALTIMESTAMP -
                                          vd_total_starttime,
                                          'DD-MON-YYYY HH24:MI:SSxFF TZH:TZM') || ']');
