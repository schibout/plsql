reate or replace PACKAGE BODY      DKA_SCTLFLUX_EAI_PKG AS
     ------------------------------------------------------------------------
     ...
     -- 15/05/2024 CAP GEMINI MAROC   Correction calcul montant crédit AR
     -- ---------- --- ------------------------------------------------------
     ...
     BEGIN
          ...
          UPDATE DKA_SCTLFLUX_EAI DSE
             SET DSE.DEBIT  = (SELECT /*SUM(NVL(DECODE(substr(RCCT.name, 6, length(RCCT.name) - 5),
                                                     'SI_AMONT_AVOIR',  (-1) * RID.AMOUNT,
                                                     'SI_AMT_ANNUL_AV', (-1) * RID.AMOUNT,
                                                     RID.AMOUNT), 0)) JJA 23/09/2015 -- DOUBLE INVERSION (avoirs deja negatifs dans Oracle) */
                                      SUM(NVL(RID.AMOUNT,0))  -- DEBIT -- KDFI-2198 : avoirs deja negatifs, SUM simple correct
                                 FROM RA_INTERFACE_LINES_ALL         RII,
                                      RA_CUST_TRX_TYPES_ALL          RCCT,
                                      RA_INTERFACE_DISTRIBUTIONS_ALL RID
        ...
        -- SEL - FIN artf07326817
          ...
          UPDATE DKA_SCTLFLUX_EAI DSE
             SET DSE.DEBIT  = (SELECT /*SUM(NVL(DECODE(SUBSTR(RCTT.NAME, 6, LENGTH(RCTT.NAME) - 5), 'SI_AMONT_AVOIR', (-1) *
                                                      RCTL.EXTENDED_AMOUNT, 'SI_AMT_ANNUL_AV', (-1) *
                                                      RCTL.EXTENDED_AMOUNT, RCTL.EXTENDED_AMOUNT), 0)) JJA 23/09/2015 -- DOUBLE INVERSION (avoirs deja negatifs dans Oracle) */
                                      SUM(NVL(RCTL.EXTENDED_AMOUNT, 0)) --DEBIT -- KDFI-2198 : avoirs deja negatifs, SUM simple correct
                                 FROM RA_CUSTOMER_TRX_LINES_ALL    RCTL,
                                      RA_CUSTOMER_TRX_ALL          RCT,
                                      RA_CUST_TRX_LINE_GL_DIST_ALL RCTLGD,
                                      RA_CUST_TRX_TYPES_ALL        RCTT,
