@echo off
rem =========================================================================
rem controleFournisseur.bat - Controle complet du flux FOURNISSEURS
rem
...
for /f "usebackq delims=" %%F in (`python "%SCRIPT_DIR%selectionner_source.py" FOURNISSEUR "%FICHIER_ENTREE%"`) do set "SOURCE_SELECTIONNEE=%%F"
if not defined SOURCE_SELECTIONNEE (
    echo [ERREUR] Aucun fichier SRC FOURNISSEUR valide n'a ete selectionne.
    exit /b 1
)
set "FICHIER_ENTREE=%SOURCE_SELECTIONNEE%"

echo =======================================================================
echo Controle complet FOURNISSEURS
